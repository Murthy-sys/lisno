...
Each calculation entry has `scope`, `source`, `settings` and `maximumDiscountBps`. For In-house, an exact selling-price cap is returned only when Quantity and the UOM scale are available; otherwise it is `null`. The established PMC/Sub-Vendor compatibility projection retains its rate-difference value. An explicitly null or missing scope never falls back to another scoped cost. Older records use the same compatibility rules as the configuration UI: an old shared value may seed each scope (`legacy_shared`), and an old In-house value may seed its two costs (`legacy_in_house`). The source is explicit so a future consumer can require review of inherited settings. Once either split In-house key exists, neither split cost inherits the old In-house value. No data is rewritten by context resolution.

## Units and calculation rules

`formulaVersion` is `mode-margin-v2`. Version 2 identifies the corrected In-house true Gross Margin and selling-price-discount contract; PMC/Sub-Vendor retain their established compatibility projection. Money is integer paise and percentages are basis points: 100 paise = ₹1, and 100 basis points = 1%. `impactBps` defaults to 1,000 whenever it is omitted; an explicit zero is retained.

Use the existing backend Mode calculator, including its rounding and overflow checks:

1. Apply Impact to the Base Rate when quantity is at or below the Low Quantity Limit.
2. Multiply the revised unit rate by quantity in the Overview UOM.
3. For In-house, divide that adjusted cost by the retained percentage for the selected Starting or Minimum Gross Margin: `price = roundHalfUp(cost × 10000 / (10000 − marginBps))`.
...
For In-house, `maximumDiscountBps = floor((selectedPrice − floorPrice) × 10000 / selectedPrice)`, with zero for a zero selected price or Minimum basis. Test simulators apply temporary `modeCalculationDiscountBps` to the rounded selling price. The combined In-house simulator applies one rate to both costs, bounded by the smaller allowance. The backend enforces both the rate limit and each rounded minimum Gross Margin floor. See [AI Estimator In-house calculations](./ai-estimator-mode-calculations.md) for the exact rounding stages, examples, reconciliation identities, and failure behavior.
...
Both source panels may be visible together, but their calculators and saved inputs remain independent. An In-house preview sums independently rounded Labor and Material true-margin prices. No combined Sub-Vendor/In-house price or estimate mutation is implied by selecting both checkboxes.

These simulator branches do not extend the `configuration` context response above. The `mode-margin-v2` context does not include or resolve `advanced.pmcMarginBps` or `advanced.subVendorMarginBps`, and its projected settings must not be treated as either margin simulator's total or discount allowance. Existing saved legacy field names remain intact. The optional Sub-Vendor margin requires no data migration or backfill.
