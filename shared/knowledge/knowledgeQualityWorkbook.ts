...
export interface QualityImportIssue { row: number | null; column?: string | undefined; message: string }
...
export function createQualityWorkbookTools(loadExcelJS: () => Promise<{ default: { Workbook: typeof import("exceljs").Workbook } }>) {
...
async function parseQualityWorkbookBuffer(buffer: ArrayBuffer, qualityOptions: QualityControlOptionCatalog = EMPTY_QUALITY_CONTROL_OPTION_CATALOG): Promise<QualityImportResult> {
  ...
  for (const issue of compatibilityIssues) {
    ...
    issues.push({ row: match ? sourceRows[Number(match[1])] ?? null : null, column: match?.[2] ? columnForField(match[2]) : undefined, message: issue.message });
  }
  if (modernSimpleWorkbook) {
    ...
    for (const issue of validateQualityParametersForSave(parameters, qualityOptions)) {
      ...
      const match = /^parameters\.(\d+)(?:\.(.+))?$/u.exec(issue.path);
      const mapped = { row: match ? sourceRows[Number(match[1])] ?? null : null, column: match?.[2] ? columnForField(match[2]) : undefined, message: issue.message };
...
async function createQualityTemplateBuffer(qualityOptions: QualityControlOptionCatalog = EMPTY_QUALITY_CONTROL_OPTION_CATALOG): Promise<ArrayBuffer> {
  const ExcelJS = await loadExcelJS();
...
async function createQualityExportBuffer(parameters: readonly KnowledgeJsonObject[], qualityOptions: QualityControlOptionCatalog = EMPTY_QUALITY_CONTROL_OPTION_CATALOG): Promise<ArrayBuffer> {
  ...
  const rows = parameters.map((parameter, index) => {
    ...
    const values: Partial<Record<Header, KnowledgeJsonValue | undefined>> = {
...
function qualityAiPrompt(basketName: string, qualityOptions: QualityControlOptionCatalog = EMPTY_QUALITY_CONTROL_OPTION_CATALOG): string {
...
return { parseQualityWorkbookBuffer, createQualityTemplateBuffer, createQualityExportBuffer, qualityAiPrompt };
}
