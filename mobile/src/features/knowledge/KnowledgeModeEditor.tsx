...
import { Button, DetailIcon, Field, IconButton } from "./knowledgeDetailUi";
...
import { ModeSection, modeStyles } from "./KnowledgeModePresentation";
...
export function KnowledgeModeEditor({ item, payload, onChange, pricingPayload, onPricingChange, overviewPayload, context, masters, catalogsReady, readOnly, onValidityChange, referencedSpecificationIds = [] }: KnowledgeModeEditorProps) {
  ...
  function calculation(scope: ModeCalculationScope) {
    ...
    return <View style={scope === "pmc" ? modeStyles.calculation : modeStyles.calculationGroup}>
      {scope !== "pmc" ? <Text accessibilityRole="header" style={modeStyles.calculationHeading}>{usesRange ? "Calculations" : MODE_CALCULATION_LABELS[scope]}</Text> : null}
      <View style={modeStyles.fieldRow}>
        <View style={modeStyles.column}><NativeDecimalField label={`${MODE_CALCULATION_LABELS[scope]} Base Rate (₹)`} value={drafts[scope].baseRate} editable={!readOnly} error={errors.baseRate} keyboardType="decimal-pad" maxLength={64} onChangeText={baseRate => change(nativeCalculationChange(payload, scope, { ...drafts[scope], baseRate }, uom.scale ?? 6))} /></View>
        <View style={modeStyles.column}><Field label="UOM" accessibilityLabel={`${MODE_CALCULATION_LABELS[scope]} UOM`} value={uom.label} editable={false} /></View>
      </View>
      <View style={modeStyles.fieldRow}>
        <View style={modeStyles.column}><NativeDecimalField label={`${MODE_CALCULATION_LABELS[scope]} Low Quantity Limit`} labelMinHeight={34} value={drafts[scope].lowQuantityLimit} editable={!readOnly} error={errors.lowQuantityLimit} keyboardType="decimal-pad" maxLength={64} onChangeText={lowQuantityLimit => change(nativeCalculationChange(payload, scope, { ...drafts[scope], lowQuantityLimit }, uom.scale ?? 6))} /></View>
        <View style={modeStyles.column}><NativeDecimalField label={`${MODE_CALCULATION_LABELS[scope]} Impact (%)`} labelMinHeight={34} value={drafts[scope].impactRate} editable={!readOnly} error={errors.impactRate} keyboardType="decimal-pad" maxLength={64} onChangeText={impactRate => change(nativeCalculationChange(payload, scope, { ...drafts[scope], impactRate }, uom.scale ?? 6))} /></View>
      </View>
      <KnowledgeText>UOM follows Overview. Impact applies at or below the quantity limit.</KnowledgeText>
      <View style={modeStyles.calculationGroup}>
      <Text accessibilityRole="header" style={modeStyles.calculationHeading}>{usesRange ? "Margin range" : "Gross margin"}</Text>
      <View style={modeStyles.fieldRow}>{usesRange ? (["minimum", "maximum"] as const).map(field => <View style={modeStyles.column} key={field}><NativeDecimalField label={`${scope === "pmc" ? "PMC" : "Lisno"} ${field === "minimum" ? "Min." : "Max."} Margin (%)`} displayLabel={`${field === "minimum" ? "Minimum" : "Maximum"} (%)`} value={typeof range[field] === "number" && Number.isSafeInteger(range[field]) && (range[field] as number) >= 0 ? formatPaiseForRupeeInput(range[field] as number) : knowledgeText(range[field])} editable={!readOnly} keyboardType="decimal-pad" maxLength={64} error={issues.find(issue => issue.path === (scope === "pmc" ? field === "minimum" ? "pmcMinimumMarginBps" : "pmcMarginBps" : field === "minimum" ? "subVendorMinimumMarginBps" : "subVendorMarginBps"))?.message} onChangeText={text => change((scope === "pmc" ? withPmcMargin : withSubVendorMargin)(payload, field, nativePercentage(text)))} /></View>)
        : ([ ["minimumRate", "Min. Gross Margin (%)"], ["startingRate", "Starting Gross Margin (%)"] ] as const).map(([field, label]) => <View style={modeStyles.column} key={field}><NativeDecimalField label={`${MODE_CALCULATION_LABELS[scope]} ${label}`} displayLabel={`${field === "minimumRate" ? "Minimum" : "Starting"} (%)`} value={drafts[scope][field]} editable={!readOnly} keyboardType="decimal-pad" maxLength={64} error={errors[field]} onChangeText={text => change(nativeCalculationChange(payload, scope, { ...drafts[scope], [field]: text }, uom.scale ?? 6))} /></View>)}</View>
      ...
      <View style={modeStyles.calculationAction}><Button label={`Test ${MODE_CALCULATION_LABELS[scope]} calculations`} variant="quiet" onPress={() => setSimulation(scope)} /></View>
    </View>;
  }
  ...
  return <View style={knowledgeStyles.stack}>
    <View style={modeStyles.surface}>
    <View style={modeStyles.setup}>
      <Text accessibilityRole="header" style={knowledgeStyles.title}>Mode</Text>
...
function NativeScopeList({ sourceLabel, list, value, readOnly, onChange }: { readonly sourceLabel: string; readonly list: KnowledgePmcScopeList; readonly value: KnowledgeJsonValue | undefined; readonly readOnly: boolean; readonly onChange: (rows: readonly KnowledgeJsonValue[]) => void }) {
  ...
  const cannotAdd = rows.length >= MAX_PMC_SCOPE_ITEMS || (value !== undefined && !Array.isArray(value));
  return <View style={modeStyles.scopeList}>
    <View style={modeStyles.scopeHeading}>
      <View style={modeStyles.scopeHeadingLabel}><Text accessibilityRole="header" style={modeStyles.scopeTitle}>{label}</Text>
      {rows.length ? <Text style={modeStyles.scopeCount}>{rows.filter(row => isKnowledgeObject(row) && row.selected === true).length} selected</Text> : null}</View>
...
function NativeDecimalField({ value = "", onChangeText, displayLabel, ...props }: ComponentProps<typeof Field> & { readonly displayLabel?: string }) {
  ...
  return <Field {...props} label={displayLabel ?? props.label.replace(/^(PMC|Sub-Vendor|In-house labor|In-house material|In-house Labour|In-house Material|Labor cost|Material cost|Lisno) /i, "")} accessibilityLabel={props.label} value={text} onChangeText={next => { setText(next); onChangeText?.(next); }} />;
}
