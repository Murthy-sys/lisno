...
export function KnowledgeModeCalculationSimulator({ initialDraft, uom, onClose }: Props) {
  const id = useId();
  ...
  async function calculate(event: FormEvent<HTMLFormElement>) {
    ...
    setAttempted(true);
    clearResult();
    if (!parsed.settings || testQuantity === undefined || !uomReady) {
  ...
  return <Dialog title="Test calculations" eyebrow="Calculation simulator"
      ...
      <div className="knowledge-dialog-body">
        <div className="knowledge-mode-simulator__fields">
          ...
          <Field id={`${id}-impact`} label="Impact">
            {(props) => <Input {...props} value="10.00%" readOnly />}
          </Field>
        ...
        <p className="knowledge-mode-calculation__hint">The 10% impact applies only below the Low Quantity Limit. Markup is added to the revised amount.</p>
        ...
        {result ? <div className="knowledge-mode-simulator__result" role="status" aria-label="Calculation results">
          <dl className="knowledge-mode-calculation__totals">
            ...
            <div><dt>Total with {markupBasis} markup</dt><dd><output aria-label={`Total with ${markupBasis} markup`}>{formatKnowledgeMoney(result.totalPaise)}</output></dd></div>
          </dl>
          <p className="knowledge-mode-calculation__hint">{result.appliedImpactBps > 0 ? "10% low-quantity impact applied." : "No low-quantity impact applied."}</p>
        </div> : null}
      </div>
