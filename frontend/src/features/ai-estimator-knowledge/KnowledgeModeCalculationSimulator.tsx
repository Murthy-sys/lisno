...
import { PMC_MARGIN_ERROR, SUB_VENDOR_MARGIN_ERROR, pmcMarginIssues, subVendorMarginRange, subVendorMarginRangeIssues } from "./knowledgePmcMargin";
...
function reconcilesMarginCalculation(calculation: MarginCalculationResult, configuredImpactBps: number, quantity: string, limit: string, scale: number) {
  ...
  const expectedDiscount = (sellingPrice * BigInt(calculation.discount?.rateBps ?? 0) + 5_000n) / 10_000n;
...
export function KnowledgeModeCalculationSimulator({ initialDraft, uom, onClose, contextLabel, scope, pmcMarginBps, subVendorMarginBps, subVendorMinimumMarginBps }: Props) {
  ...
  const configuredRangePayload = {
    ...(subVendorMarginBps === undefined ? {} : { subVendorMarginBps }),
    ...(subVendorMinimumMarginBps === undefined ? {} : { subVendorMinimumMarginBps })
  };
  const configuredRange = subVendorMarginRange(configuredRangePayload);
  const rangeIssues = subVendorMarginRangeIssues(configuredRangePayload);
  ...
  const configuredMargin = scope === "sub_vendor" ? configuredRange[marginBasis] : pmcMarginBps;
  const rangeReady = !rangeIssues.length && typeof configuredRange.minimum === "number" && typeof configuredRange.maximum === "number";
  const margin = typeof configuredMargin === "number" && (scope === "sub_vendor" ? rangeReady : !pmcMarginIssues(configuredMargin).length) ? configuredMargin : undefined;
  const marginError = usesMargin && margin === undefined ? `Close this simulator and set the ${marginLabel} Margin in the configuration. ${scope === "sub_vendor" ? rangeIssues[0]?.message ?? SUB_VENDOR_MARGIN_ERROR : PMC_MARGIN_ERROR}` : undefined;
  ...
  const inputKey = JSON.stringify({ scope, uomId: uom.id, uomScopeKey: uom.scopeKey, scale: uom.decimalScale,
    draft, quantity, discount, markupBasis, marginBasis, pmcMarginBps, subVendorMarginBps, subVendorMinimumMarginBps });
    ...
    <form id={`${id}-form`} className={`knowledge-mode-simulator${usesMargin ? " knowledge-mode-simulator--pmc" : ""}`} onSubmit={(event) => event.preventDefault()} noValidate>
      <div className="knowledge-dialog-body">
        <div className="knowledge-mode-simulator__fields">
          ...
          {scope === "sub_vendor" ? <div className="knowledge-mode-simulator__margin-range">
            {(["minimum", "maximum"] as const).map((basis) => {
              const value = configuredRange[basis];
              return <Field key={basis} id={`${id}-${basis}-margin`} label={`${basis === "minimum" ? "Min." : "Max."} Lisno Margin (%)`} error={marginError}>
          ...
          </div> : usesMargin ? <Field id={`${id}-margin`} label={`${marginLabel} Margin (%)`} error={marginError}>
        ...
        {scope === "sub_vendor" ? <fieldset className="knowledge-mode-simulator__markup">
          <legend>Calculate with</legend>
          <label><Radio name={`${id}-margin-basis`} value="maximum" checked={marginBasis === "maximum"}
            onChange={() => { clearResult(); setMarginBasis("maximum"); }} /> Max. Lisno Margin</label>
          <label><Radio name={`${id}-margin-basis`} value="minimum" checked={marginBasis === "minimum"}
            onChange={() => { clearResult(); setMarginBasis("minimum"); }} /> Min. Lisno Margin</label>
        ...
        <p className="knowledge-mode-calculation__hint">{scope === "sub_vendor"
          ...
          : usesMargin ? "Selling price = adjusted cost ÷ (1 − PMC margin %). Adjusted cost includes any low-quantity impact. Discount applies afterward. The configured Impact applies at or below the Low Quantity Limit. Update fixed values in the configuration."
        ...
        {result ? <div className="knowledge-mode-simulator__result" role="status" aria-label="Calculation results">
          {scope === "sub_vendor" ? <p>Calculated with {marginBasis === "minimum" ? "Min." : "Max."} Lisno Margin.</p> : null}
          <dl className="knowledge-mode-calculation__totals">
            {result.kind === "margin" ? <>
              ...
              <div className="knowledge-mode-simulator__subtotal"><dt>{scope === "sub_vendor" ? "Selling price before discount" : `Subtotal after ${marginLabel} margin`}</dt><dd><output aria-label={scope === "sub_vendor" ? "Selling price before discount" : `Subtotal after ${marginLabel} margin`}>{formatKnowledgeMoney(result.value.totalBeforeDiscountPaise)}</output></dd></div>
