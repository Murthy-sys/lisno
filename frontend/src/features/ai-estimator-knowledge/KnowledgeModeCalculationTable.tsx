...
import type { ModeCalculationScope } from "./knowledgeModeCalculation";
    ...
    <section className="knowledge-mode-calculation" aria-labelledby={`${id}-title`}>
      ...
      <div className="knowledge-mode-calculation__groups" role="group" aria-label="Calculation settings">
        ...
        {usesMargin ? <div className="knowledge-mode-calculation__group knowledge-mode-calculation__pmc" role="group" aria-labelledby={`${id}-margin-title`}>
          ...
          <h4 id={`${id}-markup-title`}>Gross margin</h4>
          <div className="knowledge-mode-calculation__markup-fields">
            {editableField("minimumRate", <>Min.<span className="sr-only"> Gross Margin (%)</span></>, "%")}
            {editableField("startingRate", <>Starting<span className="sr-only"> Gross Margin (%)</span></>, "%")}
          </div>
          <p className="knowledge-mode-calculation__note">Gross margin determines the selling price from the revised cost.</p>
        </div>}
      </div>
    </section>
  );
