...
import { createQualityParameter, QUALITY_MAX_PARAMETERS, validateQualityParameters } from "./knowledgeQuality";
import { QUALITY_STAGE_OPTIONS, QUALITY_TYPE_LABELS, QUALITY_METHOD_LABELS, qualityEvidenceSummary, qualityParameterKey, qualityStage, qualityStageCounts, qualityText, type QualityStageFilter } from "./knowledgeQualityPresentation";
...
export const KnowledgeQualityChecklistEditor = forwardRef<KnowledgeQualityChecklistEditorHandle, Props>(function KnowledgeQualityChecklistEditor({ parameters, savedParameters, basketName, disabled, readOnly, validationAttempt, onChange }, ref) {
  ...
  const issues = useMemo(() => validateQualityParameters(parameters), [parameters]);
  ...
  return <div className="knowledge-quality-workspace">
    ...
    <div className="knowledge-quality-table-region" role="region" aria-label="Quality parameters table" tabIndex={0}>
      <table className="knowledge-quality-table">
        ...
        <thead><tr><th scope="col">#</th><th scope="col">Parameter</th><th scope="col">Stage</th><th scope="col">Type</th><th scope="col">Acceptance criteria</th><th scope="col">Evidence required</th><th scope="col">Status</th><th scope="col">Actions</th></tr></thead>
        <tbody>{visibleRows.map(({ value, index, key }) => {
          ...
          return <tr key={key} data-expanded={expanded.has(key)} data-invalid={invalid || undefined}>
            ...
            <td className="knowledge-quality-table__detail" data-label="Evidence required">{qualityEvidenceSummary(value)}{qualityText(evidence.instructions) ? <small>{qualityText(evidence.instructions)}</small> : null}</td>
            <td className="knowledge-quality-table__status" data-label="Status"><span className={`knowledge-quality-status knowledge-quality-status--${invalid ? "invalid" : isUnsaved ? "draft" : "active"}`}>{invalid ? "Needs review" : isUnsaved ? "Unsaved" : "Active"}</span></td>
    ...
    {selected ? <ContextPanel title={readOnly ? "Quality parameter details" : "Edit quality parameter"} eyebrow={`Parameter ${selectedIndex + 1} · ${basketName}`} description={readOnly ? "This shared checklist is read-only." : "Changes stay in your checklist draft. Use Save shared checklist after reviewing your parameters."} onClose={() => setSelectedKey(null)} initialFocusRef={initialFocusRef} returnFocusRef={returnFocusRef} fallbackFocusRef={fallbackFocusRef} width="wide" className="knowledge-quality-parameter-panel" footer={<div className="knowledge-quality-parameter-panel__footer"><span>{readOnly ? "Shared across this Main Basket" : "Closing keeps your draft changes"}</span><Button onClick={() => setSelectedKey(null)}>Done</Button></div>}>
      <div ref={panelFieldsRef}>
        ...
        <KnowledgeQualityParameterFields errors={validationAttempt > 0 ? Object.fromEntries(selectedIssues.map(issue => [issue.path.replace(`parameters.${selectedIndex}.`, ""), issue.message])) : undefined} key={selectedKey} prefix={rowPrefix} value={selected} disabled={disabled || readOnly} detailed onChange={next => update(selectedKey!, next)} />
      </div>
    </ContextPanel> : null}
  </div>;
});
