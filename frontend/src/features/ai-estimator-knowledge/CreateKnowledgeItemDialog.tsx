...
export function CreateKnowledgeItemDialog({ onClose, onCreated }: {
  ...
  const valid = Boolean(!baskets.isError && activeBaskets.some((basket) => basket.id === basketId) && subBasketName.trim() && name.trim());
  const createItem = useMutation({
    mutationFn: () => createKnowledgeMainLine(basketId, { name: name.trim(), subBasketName: subBasketName.trim() }),
  ...
  return (
    <Dialog title="Add estimation item" eyebrow="Estimation configuration" onClose={onClose} busy={busy}>
      <form className="knowledge-dialog-form" onSubmit={(event) => {
        ...
        <div className="knowledge-dialog-body">
          ...
          <Field id="item-basket" label="Main basket" required>
            {(props) => <Select {...props} value={basketId} disabled={busy || !baskets.data || baskets.isError} onChange={(event) => { setBasketId(event.target.value); createItem.reset(); }}>
              <option value="">Select a basket</option>
              {activeBaskets.map((basket) => <option key={basket.id} value={basket.id}>{basket.name}</option>)}
            </Select>}
          </Field>
          <Field id="item-sub-basket" label="Sub basket" required>
          ...
          <Field id="item-name" label="Main Line name" required>
        ...
        <div className="knowledge-dialog-actions">
          ...
          <Button type="submit" busy={busy} disabled={busy || !valid}>Add estimation item</Button>
        </div>
      </form>
    </Dialog>
  );
