...
const discountedResult = { ...pmcResult, totalPaise: 194_022, finalVendorChargesPaise: 165_000,
  discount: { rateBps: 225, totalBeforeDiscountPaise: 194_690, amountPaise: 668 } };
...
describe("PMC calculation configuration and simulator", () => {
  ...
  it("keeps loading local, aborts old work, and retries only a current failure with unchanged inputs", async () => {
    ...
    await advance(300);
    expect(within(dialog).getByLabelText("Final total")).toHaveTextContent("₹1,940.22");
  ...
  it("locks configured inputs, leaves only Quantity and Discount editable, and uses the PMC request", async () => {
    ...
    for (const name of ["UOM", "Base Rate (₹)", "Low Quantity Limit", "Impact (%)", "Max. PMC Margin (%)"]) {
    ...
    expect(field("Max. PMC Margin (%)")).toHaveValue("15.25");
    expect(field("Min. PMC Margin (%)")).toHaveValue("15.25");
    expect(within(dialog).getAllByRole("textbox", { name: /PMC Margin/ })).toHaveLength(2);
    expect(within(dialog).getByRole("radio", { name: "Max. PMC Margin" })).toBeChecked();
    expect(within(dialog).getByRole("radio", { name: "Min. PMC Margin" })).not.toBeChecked();
    ...
    expect(within(dialog).getByText(/Selling price = adjusted cost ÷ \(1 − PMC margin %\)/)).toHaveTextContent(
      "Adjusted cost includes any low-quantity impact. Discount applies only to the PMC charge.");
    ...
    expect(within(dialog).getByLabelText("PMC charge")).toHaveTextContent("+₹296.90");
    expect(within(dialog).getByLabelText("Selling price before discount")).toHaveTextContent("₹1,946.90");
    expect(within(dialog).getByLabelText("Discount on PMC charge")).toHaveTextContent("−₹0.00");
  ...
  it("defaults to Max. PMC Margin and recalculates with the selected Min. value", async () => {
    setup({ pmcMinimumMarginBps: 1_250, pmcMarginBps: 1_800 });
    vi.mocked(previewKnowledge).mockResolvedValueOnce(response({ ...pmcResult,
      pmcMarginBps: 1_800, pmcMarginAmountPaise: 36_220,
      totalBeforeDiscountPaise: 201_220, totalPaise: 201_220, finalVendorChargesPaise: 165_000
    }));
    const dialog = await open();
    await settleCalculation();
    expect(previewKnowledge).toHaveBeenLastCalledWith(expect.objectContaining({
      pmcCalculation: expect.objectContaining({ pmcMarginBps: 1_800 })
    }), automaticRequestOptions);
    expect(within(dialog).getByText("Calculated with Max. PMC Margin.")).toBeVisible();
    vi.mocked(previewKnowledge).mockResolvedValueOnce(response({ ...pmcResult,
      pmcMarginBps: 1_250, pmcMarginAmountPaise: 23_571,
      totalBeforeDiscountPaise: 188_571, totalPaise: 188_571, finalVendorChargesPaise: 165_000
    }));
    await userEvent.click(within(dialog).getByRole("radio", { name: "Min. PMC Margin" }));
    expect(within(dialog).queryByRole("status", { name: "Calculation results" })).not.toBeInTheDocument();
    await settleCalculation();
    expect(previewKnowledge).toHaveBeenLastCalledWith(expect.objectContaining({
      pmcCalculation: expect.objectContaining({ pmcMarginBps: 1_250 })
    }), automaticRequestOptions);
    expect(within(dialog).getByText("Calculated with Min. PMC Margin.")).toBeVisible();
  });

  it("shows PMC margin separately and discounts only the PMC charge using the authoritative reconciled result", async () => {
    setup();
    const dialog = await open();
    expect(field("Discount (%)")).toHaveAccessibleDescription("Discount applies only to the PMC charge. Enter your custom percentage.");
    ...
    expect(within(dialog).getByLabelText("PMC charge")).toHaveTextContent("+₹296.90");
    expect(within(dialog).getByText("PMC charge (15.25%)")).toBeVisible();
    expect(within(dialog).queryByLabelText("Effective PMC margin")).not.toBeInTheDocument();
    expect(field("Discount (%)")).toHaveAccessibleDescription("Discount applies only to the PMC charge. Enter your custom percentage.");
    expect(within(dialog).getByLabelText("Selling price before discount")).toHaveTextContent("₹1,946.90");
    expect(within(dialog).getByLabelText("Discount on PMC charge")).toHaveTextContent("−₹6.68");
    expectNoVendorSummary(dialog);
    expect(within(dialog).getByLabelText("Final total")).toHaveTextContent("₹1,940.22");
    field("Quantity").focus();
  ...
  it.each([
    ...
    { name: "₹220 at 20% with 10% discount", baseRatePaise: 20_000, impactBps: 1_000, impactAmountPaise: 2_000,
      revisedAmountPaise: 22_000, pmcMarginBps: 2_000, marginAmountPaise: 5_500, subtotalPaise: 27_500,
      discountBps: 1_000, discountAmountPaise: 550, totalPaise: 26_950, balancePaise: 22_000,
      expectedMargin: "+₹55.00", expectedSubtotal: "₹275.00", expectedDiscount: "−₹5.50", expectedTotal: "₹269.50" },
    ...
    { name: "₹220 at 20% with 100% discount", baseRatePaise: 20_000, impactBps: 1_000, impactAmountPaise: 2_000,
      revisedAmountPaise: 22_000, pmcMarginBps: 2_000, marginAmountPaise: 5_500, subtotalPaise: 27_500,
      discountBps: 10_000, discountAmountPaise: 5_500, totalPaise: 22_000, balancePaise: 22_000,
      expectedMargin: "+₹55.00", expectedSubtotal: "₹275.00", expectedDiscount: "−₹55.00", expectedTotal: "₹220.00" },
    ...
    await settleCalculation();
    expect(within(dialog).queryByRole("alert")).not.toBeInTheDocument();
    expect(within(dialog).getByLabelText("PMC charge")).toHaveTextContent(sample.expectedMargin);
    expect(within(dialog).getByLabelText("Selling price before discount")).toHaveTextContent(sample.expectedSubtotal);
    expect(within(dialog).getByLabelText("Discount on PMC charge")).toHaveTextContent(sample.expectedDiscount);
  ...
  it.each([
    { percentage: "20", rateBps: 2_000, discountAmountPaise: 2_222, totalPaise: 108_889, finalVendorChargesPaise: 100_000, expectedDiscount: "−₹22.22", expectedTotal: "₹1,088.89" },
    { percentage: "50", rateBps: 5_000, discountAmountPaise: 5_556, totalPaise: 105_555, finalVendorChargesPaise: 100_000, expectedDiscount: "−₹55.56", expectedTotal: "₹1,055.55" },
    { percentage: "95", rateBps: 9_500, discountAmountPaise: 10_555, totalPaise: 100_556, finalVendorChargesPaise: 100_000, expectedDiscount: "−₹105.55", expectedTotal: "₹1,005.56" },
    { percentage: "100", rateBps: 10_000, discountAmountPaise: 11_111, totalPaise: 100_000, finalVendorChargesPaise: 100_000, expectedDiscount: "−₹111.11", expectedTotal: "₹1,000.00" }
    ...
    expect(within(dialog).queryByRole("alert")).not.toBeInTheDocument();
    expect(within(dialog).getByLabelText("PMC charge")).toHaveTextContent("+₹111.11");
    expect(within(dialog).getByLabelText("Discount on PMC charge")).toHaveTextContent(sample.expectedDiscount);
    expect(within(dialog).getByLabelText("Final total")).toHaveTextContent(sample.expectedTotal);
    expectNoVendorSummary(dialog);
    expect(props.onChange).not.toHaveBeenCalled();
  ...
  it("accepts independently rounded amounts below the former ten-percent floor", async () => {
    ...
    vi.mocked(previewKnowledge).mockResolvedValueOnce(response({ baseAmountPaise: 2_275, lowQuantityImpactAmountPaise: 0,
      ...
      totalPaise: 2_528, finalVendorChargesPaise: 2_275,
      discount: { rateBps: 20, totalBeforeDiscountPaise: 2_529, amountPaise: 1 } }));
    await settleCalculation();
    expect(within(dialog).queryByRole("alert")).not.toBeInTheDocument();
    expect(within(dialog).getByLabelText("Discount on PMC charge")).toHaveTextContent("−₹0.01");
  ...
  it.each([undefined, null, "15.25", 999, 2_001, 1_525.1])("requires a valid configured margin (%s), with no silent default", async (pmcMarginBps) => {
    setup({ pmcMarginBps });
    const dialog = await open();
    expect(field("Max. PMC Margin (%)")).toHaveAttribute("aria-invalid", "true");
    expect(within(dialog).getAllByText(/Close this simulator and set the PMC Margin range/)).toHaveLength(2);
    await settleCalculation();
    expect(previewKnowledge).not.toHaveBeenCalled();
    expect(dialog).not.toHaveTextContent(/markup/i);
  });
  ...
  it.each([
    ...
    expect(within(dialog).getByLabelText("PMC charge")).toBeVisible();
    expect(within(dialog).queryByRole("alert")).not.toBeInTheDocument();
  });
  ...
  it("applies no low-quantity charge when the configured Impact is explicitly zero", async () => {
    ...
    expect(within(dialog).queryByLabelText("Low-quantity impact amount")).not.toBeInTheDocument();
    expect(within(dialog).queryByLabelText("Additional low-quantity impact amount")).not.toBeInTheDocument();
    expect(within(dialog).getByLabelText("PMC charge")).toHaveTextContent("+₹269.91");
  });
  ...
  it.each([
    { baseRatePaise: 12_345, quantity: "2.5", impactBps: 1_250, baseAmountPaise: 30_863,
      ...
      totalBeforeDiscountPaise: 40_968, discountAmountPaise: 284, finalVendorChargesPaise: 34_720, totalPaise: 40_684,
      expectedImpact: "+₹38.57", expectedTotal: "₹406.84" },
    { baseRatePaise: 997, quantity: "0.25", impactBps: 1_000, baseAmountPaise: 249,
      ...
      totalBeforeDiscountPaise: 323, discountAmountPaise: 2, finalVendorChargesPaise: 274, totalPaise: 321,
      expectedImpact: "+₹0.25", expectedTotal: "₹3.21" }
