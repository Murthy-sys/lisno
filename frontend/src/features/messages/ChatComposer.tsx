...
import type { ChatAttachmentPolicy, ChatParticipant, ChatPriority } from "./projectChatTypes";

export function ChatComposer({ draft, participants, disabled, onChange, onSend, policy, policyError, onRetryPolicy }: {
  ...
  function keyDown(event: KeyboardEvent<HTMLTextAreaElement>) {
    ...
    if (event.key === "Enter" && !event.shiftKey && window.matchMedia("(pointer: fine)").matches) {
      event.preventDefault(); if (!sendDisabled) onSend();
    }
  }
  ...
  return <form className={`project-chat-composer${dragging ? " project-chat-composer--dragging" : ""}`} onDragOver={event => { if (event.dataTransfer.types.includes("Files")) { event.preventDefault(); setDragging(true); } }} onDragLeave={event => { if (!event.currentTarget.contains(event.relatedTarget as Node)) setDragging(false); }} onDrop={event => { event.preventDefault(); setDragging(false); addFiles(Array.from(event.dataTransfer.files)); }} aria-label="Write a project message" onSubmit={event => { event.preventDefault(); if (!sendDisabled) onSend(); }}>
    ...
    {files.length ? <ChatFileTray files={files} onRemove={id => onChange({ ...draft, files: files.filter(file => file.localId !== id) })} /> : null}
    {voice.state !== "idle" ? <div className="project-chat-recording" role="status"><span>{voice.state === "requesting" ? "Requesting microphone…" : `Recording ${Math.floor(voice.elapsed / 60)}:${String(voice.elapsed % 60).padStart(2, "0")}`}</span>{voice.state === "recording" ? <button type="button" onClick={voice.stop}><Square size={14} aria-hidden="true" /> Stop recording</button> : null}<button type="button" onClick={voice.cancel}>Cancel recording</button></div> : null}
    ...
    <div className="project-chat-composer__input" role="combobox" aria-label="Participant mentions" aria-haspopup="listbox" aria-expanded={expanded} aria-controls={expanded ? `${id}-mentions` : undefined}>
      ...
      <Textarea id={`${id}-body`} ref={input} rows={1} value={draft.body} disabled={disabled}
        ...
        onKeyDown={keyDown} onCompositionStart={() => { composing.current = true; }} onCompositionEnd={() => { composing.current = false; }}
        ...
        onChange={event => { const body = event.target.value; onChange({ ...draft, body, mentions: mentionsAfterEdit(draft.body, body, draft.mentions) }); setCursor(event.target.selectionStart); setActive(0); setDismissed(false); }} />
