...
export function LeadEstimateWorkspace() {
  ...
  return <section className="estimate-workspace" aria-labelledby="estimate-title">
    ...
    {saved.data ? <EstimateDesignUploads estimateId={saved.data.id} rooms={rooms.map((room) => ({ id: room.id, label: room.label }))} scopes={estimateBuilderSections.filter((section) => enabledSections.has(section.id)).map((section) => ({ id: section.id, label: section.label }))} /> : null}
