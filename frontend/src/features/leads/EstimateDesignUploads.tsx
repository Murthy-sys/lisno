...
export function EstimateDesignUploads({ estimateId, rooms, scopes }: EstimateDesignUploadsProps) {
  ...
  const unresolved = activeDrawings.filter((drawing) => !drawing.verified || !drawing.roomId || !drawing.scopeSectionId);
  ...
  const open = (next: DrawingSelection, nextMode: NonNullable<typeof mode>) => {
    setSelection(next);
    setMode(nextMode);
    setFormError("");
  };
    ...
    <section className="estimate-design-uploads" aria-labelledby="estimate-design-uploads-title">
      <header className="estimate-design-uploads__header">
        ...
        <form onSubmit={(event) => { event.preventDefault(); if (file) upload.mutate(file); }}>
          ...
          <input id="estimate-design-upload-file" aria-label="Design plan file" type="file" accept="application/pdf,image/png,image/jpeg,image/webp,image/tiff,image/heic" onChange={(event) => setFile(event.target.files?.[0])} />
      ...
      {selection && mode === "correct" ? <CorrectionDialog selection={selection} rooms={rooms} scopes={scopes} page={workspace.data?.pages.find((page) => page.id === selection.revision.sourcePageId) ?? workspace.data?.pages.find((page) => page.id === selection.drawing.sourcePageId)} busy={correct.isPending} error={correct.isError ? "The correction was not saved." : ""} onSubmit={(input) => correct.mutate({ ...selection, input })} onClose={closeDialog} /> : null}
...
function CorrectionDialog({ selection, rooms, scopes, page, busy, error, onSubmit, onClose }: { selection: DrawingSelection; rooms: EstimateDesignPlacementOption[]; scopes: EstimateDesignPlacementOption[]; page?: { width: number; height: number }; busy: boolean; error: string; onSubmit: (input: DrawingChange) => void; onClose: () => void }) {
