...
beforeEach(() => {
  ...
  rows = Array.from({ length: 6 }, (_, index) => ({
    ...completeVendor, id: index ? `vendor-${index + 1}` : completeVendor.id, name: index ? `Vendor ${index + 1}` : completeVendor.name,
...
describe("reference vendor directory", () => {
  it("uses independent global counts, canonical execution labels and unavailable KPI on five-row pages", async () => {
    start(); const user = userEvent.setup();
    const table = await screen.findByRole("table");
    expect(within(table).getAllByRole("rowheader")).toHaveLength(5);
    expect(within(table).getAllByText("Labor · Material + Labour")).toHaveLength(5);
    expect(within(table).getAllByText("Not available")).toHaveLength(5);
    const overview = screen.getByRole("region", { name: "Vendor overview" });
    expect(within(overview).getByText("6")).toBeVisible(); expect(within(overview).getByText("83%")).toBeVisible();
    expect(screen.getByText("Showing 1 to 5 of 6 vendors")).toBeVisible();
    expect(screen.getByRole("button", { name: "Page 1" })).toHaveAttribute("aria-current", "page");
    await user.click(screen.getByRole("button", { name: "Page 2" }));
    expect(await screen.findByText("Showing 6 to 6 of 6 vendors")).toBeVisible();
    ...
    expect(within(overview).getByText("6")).toBeVisible();
    expect(within(table).queryByText(/\d\.\d/)).not.toBeInTheDocument();
  });
  ...
  it("selects only visible rows, exposes mixed select-all, and clears selection on page/filter/reset", async () => {
    ...
    await user.click(all); expect(screen.getByText("5 vendors selected on this page")).toBeVisible(); expect(all.indeterminate).toBe(false);
    ...
    expect(await screen.findByRole("checkbox", { name: "Select Vendor 6" })).not.toBeChecked(); expect(screen.queryByText(/selected on this page/)).not.toBeInTheDocument();
    await user.click(screen.getByRole("checkbox", { name: "Select Vendor 6" }));
  ...
  it("recovers the final page after archive and refreshes global overview with reason/CAS", async () => {
    ...
    server.use(http.delete(`${endpoint}/vendor-6`, async ({ request }) => {
    ...
    await user.click(screen.getByRole("button", { name: "Page 2" }));
    await user.click(await screen.findByRole("button", { name: "Archive Vendor 6" }));
    ...
    await screen.findByText("Vendor 6 archived.");
    expect(await screen.findByText("Showing 1 to 5 of 5 vendors")).toBeVisible();
    ...
    expect(within(screen.getByRole("region", { name: "Vendor overview" })).getByText("80%")).toBeVisible();
  });
  ...
  it.each(["missing", "failed"])("keeps %s overview unavailable and independently retryable", async (state) => {
    ...
    expect(await within(overview).findByText("6")).toBeVisible();
  });
