...
describe("DesignerDetail", () => {
  ...
  it("offers only the signed-in evaluator's corrections and retains the original ISO period", async () => {
    ...
    vi.spyOn(globalThis, "fetch").mockImplementation(async (input, init) => {
      ...
      if (url === "/api/v1/designers/designer-1/audit?limit=100&offset=0&sort=desc") return Response.json({ data: { items: [], pagination: { limit: 100, offset: 0, total: 101, hasMore: true } } });
      if (url === "/api/v1/designers/designer-1/audit?limit=100&offset=100&sort=desc") return Response.json({ data: { items: [
        { id: "audit-page-two", actorId: "manager-1", action: "task_deadline_revised", entityType: "task", entityId: "task-1", occurredAt: "2026-07-01T00:00:00.000Z", oldValues: {}, newValues: {}, reason: "Second page reason", createdAt: "2026-07-01T00:00:00.000Z" }
      ], pagination: { limit: 100, offset: 100, total: 101, hasMore: false } } });
