...
export function DesignerDetail() {
  ...
  return <section className="designer-page" aria-labelledby="designer-detail-title"><Link to="/manager" className="back-link">Back to team</Link><header className="workspace-header"><div><p className="eyebrow">Designer delivery record</p><h1 id="designer-detail-title">{designer.user.name}</h1><p>Calculated KPI remains separate from manager evaluation.</p></div><strong>KPI {designer.kpi.score}</strong></header>
    ...
    <EvaluationForm subjectUserId={designerId} queryKey={managementKeys.evaluations(designerId)} />
    {revisionTask ? <DeadlineRevisionDialog task={revisionTask} onClose={() => setRevisionTaskId(null)} /> : null}
  </section>;
}
