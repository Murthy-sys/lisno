...
export function ManagementProjectWorkspace() {
  ...
  const versions = useQuery({ queryKey: ["management", "project", projectId, "versions"], queryFn: () => getManagementProjectVersions(projectId), enabled: Boolean(projectId) });
  const audit = useQuery({ queryKey: ["management", "project", projectId, "activity"], queryFn: () => getManagementProjectActivity(projectId), enabled: Boolean(projectId) });
  ...
  return <section className="designer-page" aria-labelledby="management-project-title">
    ...
    <section aria-labelledby="project-audit-title"><h2 id="project-audit-title">Project activity</h2>{audit.isPending ? <p>Loading activity…</p> : audit.isError ? <p role="alert">Activity could not be loaded.</p> : audit.data.items.length ? <ol className="activity-list">{audit.data.items.map((event) => <li key={event.id}><strong>{event.action}</strong><span>{new Date(event.occurredAt).toLocaleString()}</span></li>)}</ol> : <p>No project activity yet.</p>}</section>
  </section>;
}
