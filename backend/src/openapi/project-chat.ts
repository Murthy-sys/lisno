...
const issueAction = { type: "string", enum: ["raise", "escalate", "resolve", "reopen", "lower", "clear", "assign"] };
...
export const CHAT_COMPONENT_SCHEMAS: Record<string, Record<string, unknown>> = {
  ...
  ChatParticipant: object({ id, name: { type: "string" }, role: { type: "string", enum: ROLE_CODES }, sources: array("ChatMembershipSource"), selection: { ...object({ id, version }), nullable: true } }),
  ChatParticipantPage: object({ items: array("ChatParticipant"), setupWarnings: { type: "array", items: { type: "string" } } }),
  ...
  ChatCapabilities: object({ canSend: { type: "boolean" }, canManageParticipants: { type: "boolean" }, canManageIssues: { type: "boolean" } }),
  ChatSummary: object(summaryProperties),
  ChatConversation: object({ ...summaryProperties, lastMessageAt: { ...timestamp, nullable: true } }),
  ChatConversationPage: object({ items: array("ChatConversation"), pagination: ref("Pagination") }),
  ChatIssueHistory: object({ id, action: issueAction, actor: ref("ChatPerson"), occurredAt: timestamp, note: { type: "string" }, priority, status: issueStatus, responsibleUserId: nullableId }),
  ...
  ChatMessage: object({
    ...
    priority, issueStatus, raisedBy: nullable("ChatPerson"), responsible: nullable("ChatResponsible"), version,
    issueHistory: array("ChatIssueHistory"),
    capabilities: object(Object.fromEntries(["canRaise", "canResolve", "canReopen", "canAssign", "canAssignSelf"].map((key) => [key, { type: "boolean" }])))
  }),
  ...
  ChatSendRequest: object({ body: { type: "string", maxLength: 4000, description: "Caption may be empty only when attachmentIds contains at least one ready upload." }, attachmentIds: { type: "array", items: id, uniqueItems: true, maxItems: 10, default: [] }, mentions: { ...array("ChatMention"), maxItems: 80, description: "UTF-16 text spans; at most 20 distinct current participant IDs." }, replyToId: nullableId, priority, responsibleUserId: nullableId, clientMessageId: { ...id, minLength: 8 } }, ["body", "clientMessageId"]),
  ChatIssueRequest: object({ action: issueAction, expectedVersion: version, idempotencyKey: { ...id, minLength: 8 }, priority: { type: "string", enum: ["important", "critical"] }, responsibleUserId: nullableId, note: { type: "string", maxLength: 1000 } }, ["action", "expectedVersion", "idempotencyKey"]),
...
export const CHAT_QUERY_PARAMETERS = {
  ...
  "GET /project-messages": [query("limit", { type: "integer", minimum: 1, maximum: 50, default: 20 }), query("offset", { ...integer, maximum: 100000 })],
