...
const taxRuleSchema = new Schema(
  ...
  { collection: "aiEstimatorKnowledgeTaxRules", timestamps: true, versionKey: false, strict: "throw" }
);

taxRuleSchema.pre("validate", function normalizeMaster() {
...
taxRuleSchema.index({ codeNormalized: 1 }, { unique: true, partialFilterExpression: nonArchived });
taxRuleSchema.index({ nameNormalized: 1 }, { unique: true, partialFilterExpression: nonArchived });
taxRuleSchema.index({ status: 1, displayOrder: 1, _id: 1 });

export const AiEstimatorKnowledgeTaxRuleModel =
  models.AiEstimatorKnowledgeTaxRule ?? model("AiEstimatorKnowledgeTaxRule", taxRuleSchema);
