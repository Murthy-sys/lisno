...
const modeSchema = new Schema(
  ...
  { collection: "aiEstimatorKnowledgeModes", timestamps: true, versionKey: false, strict: "throw" }
);

modeSchema.pre("validate", function normalizeMaster() {
...
modeSchema.index({ codeNormalized: 1 }, { unique: true, partialFilterExpression: nonArchived });
modeSchema.index({ nameNormalized: 1 }, { unique: true, partialFilterExpression: nonArchived });
modeSchema.index({ status: 1, displayOrder: 1, _id: 1 });

export const AiEstimatorKnowledgeModeModel =
  models.AiEstimatorKnowledgeMode ?? model("AiEstimatorKnowledgeMode", modeSchema);
