...
const uomSchema = new Schema(
  {
    ...
    decimalScale: { type: Number, required: true, min: 0, max: 3, validate: Number.isSafeInteger },
  ...
  { collection: "aiEstimatorKnowledgeUoms", timestamps: true, versionKey: false, strict: "throw" }
);

uomSchema.pre("validate", function normalizeMaster() {
...
uomSchema.index({ codeNormalized: 1 }, { unique: true, partialFilterExpression: nonArchived });
uomSchema.index({ nameNormalized: 1 }, { unique: true, partialFilterExpression: nonArchived });
uomSchema.index({ status: 1, displayOrder: 1, _id: 1 });

export const AiEstimatorKnowledgeUomModel =
  models.AiEstimatorKnowledgeUom ?? model("AiEstimatorKnowledgeUom", uomSchema);
