...
describe("human JWT operation registry", () => {
  ...
  it("matches all 227 normative operation rows", () => {
    ...
    expect(Object.keys(HUMAN_JWT_OPERATIONS)).toHaveLength(227);
  });
  ...
  it("classifies Estimate Design rows 40 through 53 with one ordered marker pair", () => {
    ...
    expect(matches).toHaveLength(1);
    expect(matches[0]!.routes.map(({ key }) => key).sort()).toEqual(
      [...ESTIMATE_DESIGN_EXPECTED_ROUTES, "DELETE /estimate-design-uploads/:uploadId"].sort()
    );
    for (const route of matches[0]!.routes) {
      expect(route.authenticationIndices, `${route.key} authentication markers`).toHaveLength(1);
      expect(route.operationMarkers, `${route.key} operation markers`).toHaveLength(1);
  ...
  it("mounts all five Prompt 2 Admin project operations in one authenticated router", () => {
  ...
  it("appends exactly 53 AI Estimator Knowledge operations with one closed namespace", () => {
    ...
    expect(EXPECTED_AI_ESTIMATOR_KNOWLEDGE_OPERATIONS).toHaveLength(53);
    expect(HUMAN_JWT_OPERATION_LIST.filter(
    ...
    for (const key of [
      "GET /admin/ai-estimator-knowledge/baskets/:basketId/deletion-impact"
    ]) {
      expect(HUMAN_JWT_OPERATIONS[key as keyof typeof HUMAN_JWT_OPERATIONS]).toMatchObject({
        permission: "ai_estimator_knowledge.configuration.lifecycle",
        scope: { kind: "non_project", namespace: "ai_estimator_knowledge" },
  ...
  it("mounts the exact 227-operation manifest with one ordered marker pair each", () => {
    ...
    expect(expectedKeys).toHaveLength(227);
    expect(new Set(expectedKeys).size).toBe(227);
  ...
  it("has 227 unique keys and exactly 135 routed permissions", () => {
    expect(new Set(HUMAN_JWT_OPERATION_LIST.map(({ key }) => key)).size).toBe(227);
